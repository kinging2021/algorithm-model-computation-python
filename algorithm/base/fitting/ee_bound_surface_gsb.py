...
from .ee_bound_surface import EEBoundSurface

from algorithm.exception import ParamError, DataError


class EEBoundSurfaceGSB(EEBoundSurface):
    def __init__(self,
                 x,
                 y,
                 z,
        ...
        super(EEBoundSurfaceGSB, self).__init__(
            np.vstack((x, y, z)).T, x_window, y_window, min_num_sample,
            degree_x, degree_y, out_size, scale, clamped
        )
    ...
    def get_result(self):
        return {
            ...
            'z_min': self.eval_points_min[:, 2].tolist(),
            'x_max': self.eval_points_max[:, 0].tolist(),
            'y_max': self.eval_points_max[:, 1].tolist(),
            'z_max': self.eval_points_max[:, 2].tolist(),
        }
