...
class EECurveGSB(EECurve):
    def __init__(self, x, y, x_range, y_range, x_window=3,
                 bounds=None, outliers_fraction=0.005, min_num_sample=5,
                 degree=6, out_size=100, clamped=True):
    ...
    def get_result(self):
        return {
            'x_eval': self.eval_points[:, 0],
            'y_eval': self.eval_points[:, 1],
        }
...
def call(*args, **kwargs):
    ...
    for p in ['x', 'y', 'x_window', 'x_range', 'y_range']:
